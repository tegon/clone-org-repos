...
class GitHub {
  ...
  async cloneRepositories () {
    ...
    const filteredRepositories = this.options.cloneFilter(allRepositories)
  ...
  async fetchExisting () {
    ...
    const filteredRepositories = this.options.fetchFilter(this.options.existingGitRepos)
    ...
    if (this.options.fetch === 0 || filteredRepositories.length === 0) {
  ...
  async fetch (repo, callback) {
    ...
    process.on('close', (status) => {
      if (status === 0) {
        this.options.DEBUG(`${repo.name} fetch succeeded`)
      } else {
        this.options.ERROR(`\t${repo.name} fetch failed with status ${status}`)
      }
      callback(null, true)
    })
  }
